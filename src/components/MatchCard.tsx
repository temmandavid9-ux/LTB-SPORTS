...
export const MatchCard: React.FC<MatchCardProps> = ({ 
  ...
  const isFavoriteTeam = favoriteTeam && (
    match.home_team.name.toLowerCase().includes(favoriteTeam.toLowerCase()) ||
    match.away_team.name.toLowerCase().includes(favoriteTeam.toLowerCase())
  );
  ...
  const shareMatch = () => {
    const text = `Watch ${match.home_team.name} vs ${match.away_team.name} live on LTB Sports! ${globalThis.location.href}`;
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl overflow-hidden shadow-xl border transition-all relative ${
        isFavoriteTeam 
          ? 'bg-slate-800 border-red-500 ring-2 ring-red-500/20 shadow-red-500/10' 
        ...
        <div className="flex items-center gap-3">
          <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border transition-all ${
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">
              {match.status.type === 'inprogress' ? 'IN PLAY' : match.status.description}
            </span>
            {match.status.type === 'inprogress' && (
              ...
              <div className="bg-slate-900 p-4 rounded-2xl border border-slate-700">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {broadcasters.map(b => (
                    <button
                      key={b.name}
                      onClick={() => onLtbChannelClick?.(b.url)}
        ...
        <div className="flex gap-2">
          {match.status.type === 'inprogress' ? (
            <div className="flex-[3] flex gap-2">
              ...
              >
                <Video size={14} /> Highlights
              </button>
              <button
                onClick={() => {
                  if (!showLive && onWatchStream) {
                    onWatchStream(match);
                  } else {
              ...
              >
                ...
                {showLive ? 'CLOSE STREAM' : 'WATCH STREAM'}
              </button>
            </div>
            ...
            <div className="flex-1 flex gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-xl transition-all font-bold text-xs ${
                  isPending ? 'bg-blue-600 text-white animate-pulse' : 'bg-red-600 hover:bg-red-700 text-white'
                }`}
              >
